...
PromptWheel supports multiple AI backends. Choose the one that fits your workflow.
...
PromptWheel uses the official `codex exec` CLI. Not all models are available with `codex login` (ChatGPT subscription):
...
These restrictions are enforced by OpenAI's Codex CLI, not PromptWheel. If your saved model becomes incompatible (e.g., you switch from API key to `codex login`), PromptWheel will prompt you to re-select.

### Changing your saved model

```bash
promptwheel --codex --codex-model <name>
```

### Can third-party tools bypass these restrictions?

Some third-party tools intercept Codex OAuth tokens to access restricted models. **This likely violates OpenAI's Terms of Service** — users have reported account bans for similar approaches with other providers. PromptWheel only uses the official `codex exec` CLI and respects its model restrictions.
...
promptwheel --local --local-model deepseek-coder-v2 --local-url http://localhost:8080/v1

# Limit agentic loop iterations (default: 20)
promptwheel --local --local-model qwen2.5-coder --local-max-iterations 10
```
...
```bash
promptwheel --scout-backend codex
```
...
```bash
promptwheel --model sonnet    # Use Sonnet (faster, cheaper)
promptwheel --model haiku     # Use Haiku (fastest, cheapest)
promptwheel --model opus      # Use Opus (default, highest quality)
```

For Codex runs:

```bash
promptwheel --codex --codex-model gpt-5.2-codex
```

For local runs:

```bash
promptwheel --local --local-model <model-name>
```
